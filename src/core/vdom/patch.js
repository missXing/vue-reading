...
function sameVnode (a, b) {
  return (
    ...
    a.asyncFactory === b.asyncFactory && (
      // 对于同步组件，则判断 isComment、data、input 类型等是否相同
      (
        a.tag === b.tag &&
...
export function createPatchFunction (backend) {
  ...
  // 重要的方法 作用：通过虚拟节点创建真实dom 并插入到他的父节点中
  function createElm (
    vnode,
    insertedVnodeQueue,
    parentElm,
    ...
    if (isDef(tag)) {
      ...
      if (__WEEX__) {
        ...
        // 把 DOM 插入到父节点中，因为是递归调用，子元素会优先调用 insert，所以整个 vnode 树节点的插入顺序是先子后
        insert(parentElm, vnode.elm, refElm)
      }
  ...
  function createComponent (vnode, insertedVnodeQueue, parentElm, refElm) {
    let i = vnode.data
    if (isDef(i)) {
      // 第一次渲染的时候，vnode.componentInstance 为 undefined
  ...
  // 调用一些 nodeOps 把子节点插入到父节点中，insertBefore appendChild这些辅助方法定义在 src/platforms/web/runtime/node-ops.js 
  ...
  // 在整个 pathVnode 过程中，最复杂的就是 updateChildren 方法了
  ...
  function patchVnode (
    ...
    if (isUndef(vnode.text)) {
      // oldCh 与 ch 都存在且不相同时，使用 updateChildren 函数来更新子节点, 最复杂的更新
